...
**ความปลอดภัย** — dashboard มีรหัสผ่านของตัวเองแล้ว (ข้อ 2.8 — **ต้องตั้งก่อนใช้งาน**) และคู่มือนี้
ยังตั้งให้ **เข้าได้เฉพาะเครื่องที่อยู่ใน tailnet ของคุณ** เท่านั้น ไม่มี URL สาธารณะ
สองชั้นซ้อนกัน: Tailscale กันคนนอกออกไป รหัสผ่านกันเครื่องที่อยู่ในวงแต่ไม่ควรสั่งอุปกรณ์
...
| Tailscale Personal | **6 users, อุปกรณ์ไม่จำกัด**, ฟรีถาวร | Serve (เฉพาะใน tailnet) ใช้ได้ · Funnel (เปิด public) ก็ฟรี (beta) แต่ควรเพิ่ม 2FA ก่อนเปิดจริง — ดู "เปิดจากเครื่องที่ไม่มี Tailscale" ข้อ 6 |
...
```bash
cat > .env <<'EOF'
# ชื่อ tailnet ของ Pi + IP ในบ้าน — กันเว็บอื่นยิงคำสั่งมาผ่าน DNS rebinding
...
### 2.8 ตั้งรหัสผ่าน — ต้องทำ ไม่ใช่ทางเลือก

**dashboard จะไม่ให้เข้าถึงอะไรเลยจนกว่าจะตั้งรหัสผ่าน** ตั้งใจให้เป็นแบบนั้น: อัปเดตโค้ดมาแล้ว
เปิดหน้าเว็บจะเห็นหน้าบอกให้ตั้งรหัสผ่าน ไม่ใช่ dashboard — ไม่มีช่วงเวลาที่ระบบเปิดโล่ง

```bash
docker exec -it iot-app python scripts/set_password.py
```

พิมพ์รหัสผ่านสองครั้ง (อย่างน้อย 10 ตัวอักษร ไม่แสดงบนหน้าจอ) แล้ว**รีเฟรชหน้าเว็บ** —
ไม่ต้องรีสตาร์ต container แอปอ่านไฟล์ใหม่ให้เอง

เก็บไว้ที่ `/app/data/auth.json` ใน volume `app-data` เดียวกับชื่ออุปกรณ์ — **ไม่ได้เก็บใน
database** เพราะถ้าเก็บไว้บน Supabase แล้วเน็ตหลุด คุณจะเข้าไปปิดไฟในบ้านตัวเองไม่ได้
ล็อกอินต้องทำงานได้แม้ประวัติจะหายไปชั่วคราว

**ที่ได้มา:**

- ทุก route และ WebSocket ต้องมี session · route ที่เพิ่มในอนาคตถูกปิดไว้เป็นค่าเริ่มต้น
- cookie เป็น `HttpOnly` + `SameSite=Strict` และเป็น `Secure` เมื่อเข้าทาง https
- รหัสผ่านเก็บเป็น scrypt (16 MB, ~200 ms บน Pi 3B) ไม่ใช่ hash ธรรมดา
- เดารหัสผิดเกิน 10 ครั้งใน 15 นาที ต่อ IP → ตอบ 429 พร้อมบอกเวลาที่ต้องรอ
  (จำกัดต่อ IP ไม่ใช่ทั้งระบบ เพราะไม่อย่างนั้นใครก็ล็อกคุณออกจากบ้านตัวเองได้)
- `/healthz` ยังเปิดอยู่ให้ Docker เช็คสุขภาพได้ แต่ถ้าไม่ได้ล็อกอินจะเห็นแค่ `{"status":"ok"}`
  ไม่เห็นว่ามีอุปกรณ์อะไรกี่ตัว
- ค้างล็อกอินไว้ 30 วัน (`SESSION_HOURS`)

**ออกจากระบบ** — ปุ่ม "ออกจากระบบ" มุมขวาบน ยกเลิก session ที่ฝั่งเซิร์ฟเวอร์ด้วย
ไม่ใช่แค่ลบ cookie ในเบราว์เซอร์ — cookie ที่อาจถูกคัดลอกไปก่อนหน้าจะใช้ไม่ได้ทันที
เครื่องอื่นที่ล็อกอินอยู่ไม่กระทบ

**ลืมออกจากระบบบนเครื่องอื่น** — กดไอคอนคน → "ออกจากระบบทุกเครื่อง" ทุกเครื่องหลุดรวมเครื่องนี้
โดยรหัสผ่านไม่เปลี่ยน

**เปลี่ยนรหัสผ่าน** — กดไอคอนคนมุมขวาบนของ dashboard หรือสั่ง `set_password.py` อีกครั้ง
ทั้งสองทางจะทำให้เบราว์เซอร์อื่นทุกเครื่องหลุดออกจากระบบ

**ลืมรหัสผ่าน** — ลบไฟล์แล้วตั้งใหม่ (ต้องมี shell บน Pi ซึ่งก็คือเงื่อนไขความปลอดภัยของมันเอง):

```bash
docker exec -it iot-app rm /app/data/auth.json
docker exec -it iot-app python scripts/set_password.py
```

> ล็อกอินนี้เป็น**ชั้นป้องกันซ้อน** ไม่ใช่ชั้นเดียว — Tailscale ยังเป็นตัวกันหลักอยู่
> ถ้าวันหนึ่งจะเปิด Funnel ให้เป็น URL สาธารณะจริง ควรเพิ่ม 2FA ก่อน (ดูรายการ "ยังไม่ได้ทำ"
> ใน README)
...
แอปมีรหัสผ่านแล้ว (ข้อ 2.8) แต่ **รหัสผ่านเดียวยังไม่พอสำหรับ URL สาธารณะ** — และไม่ต้องรอให้ใครบังเอิญเจอ
ด้วย: cert HTTPS ทุกใบถูกประกาศลง Certificate Transparency log สาธารณะ บอทสแกนชื่อโดเมนใหม่จากตรงนั้น
ภายในไม่กี่นาที แล้วเริ่มเดารหัสทันที (ตัวจำกัดจะกันไว้ แต่ก็มีแค่ชั้นเดียว)
